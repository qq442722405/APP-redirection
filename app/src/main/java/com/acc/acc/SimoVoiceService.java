...
public final class SimoVoiceService extends Service {
    ...
    private static boolean requested(Context c){return c.getSharedPreferences(MainActivity.PREF,0).getBoolean("simo_enabled",false)||LabSession.voiceActive();}
    ...
    private final SharedPreferences.OnSharedPreferenceChangeListener changed=(p,key)->{
        ...
        if(key.equals("simo_enabled")||key.equals(MainActivity.APPS)||key.equals("simo_all_apps")||key.startsWith("simo_alias_")||key.startsWith("simo_exclude_"))refreshSoon();
    };
    ...
    private void publish(){
        ...
        try{
            ...
            VoiceCommands next=new VoiceCommands(base,LabSession.commands());
    ...
    private void handle(Bundle input){
        ...
        try{
            ...
            boolean allowed=command!=null&&(command.action.equals("open")?prefs.getBoolean("simo_enabled",false):LabSession.accepts(command));
            if(allowed&&(owner.isEmpty()||owner.equals(getPackageName()))&&(panel.isEmpty()||panel.equals(SimoVoiceProtocol.PANEL))&&(action.isEmpty()||action.equals("visoClick"))){
                ...
                else if(command.action.startsWith("lab.")){
                    LabLog.write("SIMO","CALLBACK "+command.phrase);
                    boolean submitted=LabActions.run(this,command.action.substring(4));
                    lastCommand=command.id;lastDispatch=now;lastHit="测试口令："+command.phrase+(submitted?"；请求已提交，等待实车确认":"；请求失败，请查看测试日志");result=submitted?0:-100;
