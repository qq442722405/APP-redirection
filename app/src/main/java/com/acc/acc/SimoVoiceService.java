...
public final class SimoVoiceService extends Service {
    ...
    static volatile long callbackCount,closeMatchCount;
    ...
    private void publish(){
        ...
        try{
            ...
            CloseTestLog.add("口令登记",status+"；关闭开关="+prefs.getBoolean("simo_close_enabled",false));
    ...
    private void handle(Bundle input){
        ...
        try{
            callbackCount++;
            CloseTestLog.add("收到VISO","ID="+string(input,"viso_selfid","viewWordId")+"；口令="+string(input,"viso_hotword","hotWord")+"；动作="+string(input,"viso_action","action"));
            ...
            if(allowed&&(owner.isEmpty()||owner.equals(getPackageName()))&&(panel.isEmpty()||panel.equals(SimoVoiceProtocol.PANEL))&&(action.isEmpty()||action.equals("visoClick"))){
                ...
                if(command.id.equals(lastCommand)&&now-lastDispatch<1500){result=lastResult;CloseTestLog.add("去重","1.5 秒内重复回调，未重复执行；沿用返回码="+lastResult);}
                else if(command.action.equals("close")){
                    closeMatchCount++;CloseTestLog.add("匹配关闭",command.phrase+" → "+command.pkg+"；方式="+CloseTestModes.label(SimoClose.methodFor(command.pkg)));
                    boolean submitted=SimoClose.close(this,command.pkg);
                    lastCommand=command.id;lastDispatch=now;result=submitted?0:-100;lastResult=result;
                    ...
                    lastCommand=command.id;lastDispatch=now;lastHit="收到："+command.phrase+"；已请求启动";result=0;lastResult=result;
            ...
            }else {lastHit="回调未执行："+(command==null?"口令未匹配":!allowed?"开关未启用":"页面、包名或动作不匹配");CloseTestLog.add("未执行",lastHit+"；owner="+owner+"；page="+panel+"；action="+action);}
        }catch(RuntimeException e){lastHit="语音请求失败："+e.getClass().getSimpleName();CloseTestLog.add("异常",lastHit);}
