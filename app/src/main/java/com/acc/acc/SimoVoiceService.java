...
public final class SimoVoiceService extends Service {
    ...
    static void sync(Context context){
        ...
        if(!context.getSharedPreferences(MainActivity.PREF,0).getBoolean("simo_enabled",false)){context.stopService(intent);status="未开启";return;}
    ...
    private final SharedPreferences.OnSharedPreferenceChangeListener changed=(p,key)->{
        if(key==null)return;
        if(key.equals("simo_enabled")&&!p.getBoolean(key,false)){registry=new VoiceCommands(Collections.emptyList());stopSelf();return;}
        if(key.equals(MainActivity.APPS)||key.equals("simo_all_apps")||key.startsWith("simo_alias_")||key.startsWith("simo_exclude_"))refreshSoon();
    };
    ...
    @Override public int onStartCommand(Intent intent,int flags,int id){
        if(!prefs.getBoolean("simo_enabled",false)){stopSelf();return START_NOT_STICKY;}
        if(remote==null)connect();else refreshSoon();return START_STICKY;
    }
    private void connect(){
        if(destroyed||bound||!prefs.getBoolean("simo_enabled",false))return;
        try{
    ...
    private void publish(){
        IBinder binder=remote;if(destroyed||binder==null||!prefs.getBoolean("simo_enabled",false))return;
        try{
            VoiceCommands next=new VoiceCommands(SimoApps.list(this,false));
            ...
            registeredPage=null;
            if(destroyed||binder!=remote||!prefs.getBoolean("simo_enabled",false))return;
    ...
    private void handle(Bundle input){
        ...
        try{
            ...
            if(prefs.getBoolean("simo_enabled",false)&&command!=null&&(owner.isEmpty()||owner.equals(getPackageName()))&&(panel.isEmpty()||panel.equals(SimoVoiceProtocol.PANEL))&&(action.isEmpty()||action.equals("visoClick"))){
