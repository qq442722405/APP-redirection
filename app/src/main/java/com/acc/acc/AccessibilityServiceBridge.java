...
public class AccessibilityServiceBridge extends AccessibilityService {
    ...
    static boolean closeVisibleWindow(String pkg,CloseResult result){
        ...
        try{
            ...
            if(found==null||matches!=1){result.result("未关闭：没有找到唯一可见的目标 APP 窗口");return false;}
            ...
            if(coords==null){result.result("未关闭：窗口跨区域或坐标不可靠");return false;}
            ...
            for(int finger=-1;finger<=1;finger++){Path path=new Path();float x=coords[0]+finger*coords[3];path.moveTo(x,coords[1]);path.lineTo(x,coords[2]);builder.addStroke(new GestureDescription.StrokeDescription(path,0,380));}
            ...
            result.result(accepted?"已发送目标窗口三指下滑："+pkg:"系统拒绝关闭手势："+pkg);return accepted;
