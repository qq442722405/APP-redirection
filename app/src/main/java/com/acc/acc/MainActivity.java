...
public class MainActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState){
        ...
        for(String key:prefs.getAll().keySet())if(key.startsWith("vehicle_"))defaults.remove(key);
    ...
    void handleSimoLaunch(Intent intent){
        ...
        if(intent.getBooleanExtra("manage_floating",false)){intent.removeExtra("manage_floating");getWindow().getDecorView().post(this::showFloatingWindowSettingsDialog);}
    ...
    @Override protected void onResume(){
        ...
        if(!now)dismissFloatingPresetPicker();
    }
    ...
    void showMainItemMenu(final int type, final int index){
        ...
        AlertDialog dlg=new InAppDialog.Builder(this)
                .setTitle("操作："+name)
    ...
    void deleteMainItem(int type,int index){
        if(type==0){
            ...
            new InAppDialog.Builder(this).setTitle("删除窗口预设")
            ...
            new InAppDialog.Builder(this).setTitle("删除 APP")
    ...
    @Override public void onBackPressed(){if(!InAppDialog.back(this))super.onBackPressed();}
    @Override protected void onDestroy(){dismissFloatingPresetPicker();InAppDialog.clear(this);super.onDestroy();}
    ...
    void showFloatingWindowSettingsDialog(){
        ...
        AlertDialog dialog=new InAppDialog.Builder(this)
                .setTitle("悬浮窗口设置")
                .setView(root)
                .create();
    ...
    void addCurrentButtonChip(LinearLayout row,String label,Runnable deleteAction){
        ...
        chip.setOnLongClickListener(v->{
            new InAppDialog.Builder(this).setTitle("删除悬浮窗按钮").setMessage("是否删除“"+label+"”按钮？")
    ...
    private View floatingPresetPicker;
    void dismissFloatingPresetPicker(){
        if(floatingPresetPicker==null)return;
        try{((WindowManager)getSystemService(WINDOW_SERVICE)).removeView(floatingPresetPicker);}catch(RuntimeException ignored){}
        floatingPresetPicker=null;
    }
    interface PresetBoundsCallback { void apply(int x,int y,int width,int height); }
    void showFloatingPresetPicker(Preset draft,PresetBoundsCallback callback){
        dismissFloatingPresetPicker();
        if(Build.VERSION.SDK_INT>=23 && !Settings.canDrawOverlays(this)){
        ...
        cancel.setOnClickListener(v->dismissFloatingPresetPicker());
        confirm.setOnClickListener(v->{
            ...
            try{pickerWm.updateViewLayout(picker,lp);}catch(Exception ignored){}
            dismissFloatingPresetPicker();
            callback.apply(Math.max(0,lp.x),Math.max(0,lp.y),w,h);
        });

        try{pickerWm.addView(picker,lp);floatingPresetPicker=picker;}
    ...
    void showPresetEditor(int index,Preset old){
        ...
        save.setOnClickListener(v->{
            ...
            if(p.mode!=6&&(p.w<=0||p.h<=0)){Toast.makeText(this,"非全屏预设的宽度和高度必须大于 0",Toast.LENGTH_SHORT).show();return;}
        ...
        design.overlayGate(layout.action(46,()->{
            Preset draft=new Preset(name.getText().toString(),number(x,0),number(y,0),number(width,600),number(height,400),-1,modeHolder[0],categoryHolder[0]);
            showFloatingPresetPicker(draft,(left,top,w,h)->{
                if(!dialog.isShowing())return;
                x.setText(String.valueOf(left));y.setText(String.valueOf(top));
                width.setText(String.valueOf(w));height.setText(String.valueOf(h));
                if(modeHolder[0]==6){modeHolder[0]=1;for(int i=0;i<6;i++)layout.selected(modeButtons[i],i==0);}
            });
        }));
        ...
        design.show(dialog,layout);
        dialog.setOnDismissListener(d->{dismissFloatingPresetPicker();design.openDialogs.remove(dialog);});
    }
    ...
    void showScreenDiagnostics(){
        ...
        AlertDialog dialog=new InAppDialog.Builder(this).setTitle("权限与诊断")
    ...
    void showAddAutoTaskDialog(JSONArray[] tasks,Runnable refresh){
        ...
        AlertDialog dlg=new InAppDialog.Builder(this).setTitle("添加自动启动任务").setView(root).create();
    ...
    void showAppChoiceDialog(AppChoice callback){
        ...
        AlertDialog dialog=new InAppDialog.Builder(this).setTitle("选择 APP").setView(box).setNegativeButton("关闭",null).create();
        dialogRef[0]=dialog;
    ...
    void launchIntentWithBounds(Intent intent,String pkg,int x,int y,int w,int h,int displayId,boolean fullscreen,String name){
        int target=displayId<0?getWindow().getWindowManager().getDefaultDisplay().getDisplayId():displayId;
        try{
            android.graphics.Rect bounds=WindowLaunch.start(this,intent,x,y,w,h,target,fullscreen,true);
            saveAppBounds(pkg,bounds.left,bounds.top,bounds.width(),bounds.height(),target,fullscreen);
            info.setText("已请求窗口："+name+"  "+bounds.width()+" × "+bounds.height()+"  左 "+bounds.left+"  上 "+bounds.top);
        }catch(Exception e){
            info.setText("按窗口启动失败："+e.getMessage());
            Toast.makeText(this,"按窗口启动失败，未改为全屏启动："+e.getMessage(),Toast.LENGTH_LONG).show();
        }
    }
    ...
    void launchApp(Preset p){
        ...
        launchIntentWithBounds(intent,selectedPackage,p.x,p.y,p.w,p.h,p.displayId,p.mode==6,selectedName);
    }
}
