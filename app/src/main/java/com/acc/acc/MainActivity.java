...
public class MainActivity extends AppCompatActivity {
    ...
    // Design units are pixels at 160dpi, not Android dp. Never use these for launch bounds.
    int availableWidth, availableHeight;
    int insetTop, insetBottom;
    float uiScale(){
        Point size=availableUiSize();
        if(mainUiContext){
            float saved=prefs==null?1f:prefs.getFloat("ui_scale",1f);
            return Math.max(.75f,Math.min(1.6f,Math.min(size.x/2032f,size.y/960f)))
                    * Math.max(.5f,Math.min(3f,saved));
        }
        return Math.max(.65f,Math.min(1f,Math.min(size.x/1000f,size.y/700f)));
    }

    Point availableUiSize(){
        if(availableWidth>0 && availableHeight>0) return new Point(availableWidth,availableHeight);
        if(Build.VERSION.SDK_INT>=30){
            ...
            return new Point(Math.max(1,b.width()-bars.left-bars.right),Math.max(1,b.height()-bars.top-bars.bottom));
        }
        Point p=new Point(); getWindowManager().getDefaultDisplay().getSize(p); return p;
    }

    int pickerColumns(){
        return Math.max(1,Math.min(5,(Math.min(1000,Math.round(availableUiSize().x/uiScale()))-48)/156));
    }
    ...
    TextView mainText(String s,float size){
        TextView t=new TextView(this);
        t.setText(s); t.setTextColor(Color.WHITE);
        boolean previous=mainUiContext; mainUiContext=true;
        t.setTextSize(android.util.TypedValue.COMPLEX_UNIT_PX,size*mainFontScale()*uiScale()*getResources().getConfiguration().fontScale);
        mainUiContext=previous;
        t.setTag("design-px");
        t.setGravity(Gravity.CENTER_VERTICAL);
        return t;
    }
    ...
    void adaptViewBoxes(ViewGroup group){
        for(int i=0;i<group.getChildCount();i++){
            ...
            if(v instanceof TextView){
                ...
                if(!"design-px".equals(tv.getTag())){
                    float density=getResources().getDisplayMetrics().density;
                    tv.setTextSize(android.util.TypedValue.COMPLEX_UNIT_PX,tv.getTextSize()/density*uiScale());
                    tv.setTag("design-px");
                }
                if(tv instanceof Button || tv instanceof EditText){
                    tv.setMinimumHeight(dp(48));
                    if(tv instanceof Button){tv.setMaxLines(2);tv.setEllipsize(android.text.TextUtils.TruncateAt.END);}
                }
            }
            if(v.isClickable()){v.setFocusable(true);if(v.getContentDescription()==null && v instanceof TextView)v.setContentDescription(((TextView)v).getText());}
    ...
    @Override protected void onCreate(Bundle savedInstanceState){
        ...
        if(!prefs.contains("main_app_columns")) defaults.putInt("main_app_columns",8);
    ...
    void buildUI(){
        ...
        mainFrame=frame;
        frame.setBackgroundColor(0xFF101923);
        ...
        // System bars are already excluded by the content host; reserve only the remainder.
        int topReserve=Math.max(0,dp(topBlank)-insetTop);
        root.setPadding(dp(30),topReserve+dp(30),dp(30),dp(10));
        ...
        FrameLayout.LayoutParams scrollArea=new FrameLayout.LayoutParams(-1,-1);
        int viewportHeight=availableUiSize().y;
        int footerHeight=Math.min(dp(77),Math.max(48,viewportHeight/4));
        int bottomReserve=Math.min(Math.max(0,dp(120)-insetBottom),Math.max(0,viewportHeight/5));
        scrollArea.bottomMargin=bottomReserve+footerHeight+dp(7);
        frame.addView(mainScroll,scrollArea);
        ...
        addPreset.setTextSize(35*mainFontScale());
        addPreset.setContentDescription("新建窗口预设");
        addPreset.setOnClickListener(v->editPreset(-1));
        presetHeader.addView(addPreset,new LinearLayout.LayoutParams(dp(70),dp(70)));
        TextView pt=mainText("窗口预设",60); pt.setTypeface(null,1);
        LinearLayout.LayoutParams titleLp=new LinearLayout.LayoutParams(dp(240),dp(70));titleLp.leftMargin=dp(20);titleLp.rightMargin=dp(5);
        presetHeader.addView(pt,titleLp);

        // 左 / 中 / 右分类按钮：放在“窗口预设”和“悬浮窗选位”之间，靠左排列，每个宽 50。
        presetCategoryButtons=new Button[3];
        String[] presetCats={"左","中","右"};
        for(int ci=0;ci<3;ci++){
            final int cci=ci;
            Button cb=button(presetCats[ci]);
            cb.setTextSize(35*mainFontScale());
            cb.setPadding(0,0,0,0);cb.setMinWidth(0);
            ...
            LinearLayout.LayoutParams clp=new LinearLayout.LayoutParams(dp(70),dp(70));
            clp.setMargins(0,0,dp(20),0);
            presetHeader.addView(cb,clp);
        }
        ...
        if(hasOverlayPermission()){
            ...
            floatingPick.setTextSize(35*mainFontScale());
            floatingPick.setPadding(0,0,0,0);
            floatingPick.setContentDescription("悬浮窗选位");
            floatingPick.setOnClickListener(v->showFloatingPresetPicker());
            LinearLayout.LayoutParams pickLp=new LinearLayout.LayoutParams(dp(200),dp(68));
            pickLp.setMargins(0,0,0,0);
            presetHeader.addView(floatingPick,pickLp);
        }
        LinearLayout.LayoutParams ph=new LinearLayout.LayoutParams(-1,dp(70));
        ph.bottomMargin=dp(30);
        HorizontalScrollView headerScroll=new HorizontalScrollView(this);
        headerScroll.addView(presetHeader,new HorizontalScrollView.LayoutParams(-2,-1));
        root.addView(headerScroll,ph);
        ...
        root.addView(presetScroll,new LinearLayout.LayoutParams(-1,dp(220)));
        ...
        addApp.setTextSize(35*mainFontScale());
        addApp.setContentDescription("添加 APP");
        addApp.setOnClickListener(v->chooseApp());
        appHeader.addView(addApp,new LinearLayout.LayoutParams(dp(70),dp(70)));
        TextView at=mainText("APP",60); at.setTypeface(null,1);
        LinearLayout.LayoutParams appTitleLp=new LinearLayout.LayoutParams(0,dp(70),1);appTitleLp.leftMargin=dp(20);
        appHeader.addView(at,appTitleLp);
        LinearLayout.LayoutParams ah=new LinearLayout.LayoutParams(-1,dp(70));
        ah.topMargin=dp(15); ah.bottomMargin=dp(30);
        root.addView(appHeader,ah);
        ...
        appScroll.addView(appGrid,new HorizontalScrollView.LayoutParams(-2,-2));
        int appColumns=Math.max(1,prefs.getInt("main_app_columns",8));
        int appRows=Math.max(1,(apps.size()+appColumns-1)/appColumns);
        // Wrap content so adding/removing an APP across a row boundary updates scroll extent.
        root.addView(appScroll,new LinearLayout.LayoutParams(-1,-2));
        ...
        // Bottom actions are a sibling of the scrolling lists, anchored at (30,763) at baseline.
        rightFooter.removeView(actionRow);
        rightFooter.removeView(screenInfo);
        footer.removeAllViews();
        footer.setPadding(dp(30),0,dp(30),0);
        footer.setGravity(Gravity.TOP|Gravity.LEFT);
        footer.setBackgroundColor(0xFF101923);
        actionRow.setGravity(Gravity.LEFT|Gravity.TOP);
        for(int i=0;i<actionRow.getChildCount();i++){
            View action=actionRow.getChildAt(i);
            LinearLayout.LayoutParams a=new LinearLayout.LayoutParams(dp(100),Math.min(dp(70),footerHeight));
            a.rightMargin=dp(20); action.setLayoutParams(a); action.setFocusable(true);
        }
        settings.setText("设置");
        HorizontalScrollView actionScroll=new HorizontalScrollView(this);
        actionScroll.addView(actionRow);
        footer.addView(actionScroll,new LinearLayout.LayoutParams(Math.max(1,Math.min(dp(480),availableUiSize().x-dp(60))),footerHeight));
        LinearLayout status=new LinearLayout(this); status.setOrientation(LinearLayout.VERTICAL);
        info.setMaxLines(2); info.setEllipsize(android.text.TextUtils.TruncateAt.END);
        status.addView(info,new LinearLayout.LayoutParams(-1,0,3));
        status.addView(screenInfo,new LinearLayout.LayoutParams(-1,0,2));
        // On narrow/very short windows hide secondary telemetry explicitly, never crop actions.
        if(availableUiSize().x<dp(800) || footerHeight<dp(70))status.setVisibility(View.GONE);
        footer.addView(status,new LinearLayout.LayoutParams(0,footerHeight,1));
        FrameLayout.LayoutParams footerArea=new FrameLayout.LayoutParams(-1,footerHeight,Gravity.BOTTOM);
        footerArea.bottomMargin=bottomReserve;
        frame.addView(footer,footerArea);
        frame.setOnApplyWindowInsetsListener((v,insets)->{
            int top=insets.getSystemWindowInsetTop(), bottom=insets.getSystemWindowInsetBottom();
            if(top!=insetTop || bottom!=insetBottom){ insetTop=top; insetBottom=bottom; v.post(()->buildUI()); }
            return insets;
        });
        frame.addOnLayoutChangeListener((v,l,t,r,b,ol,ot,or,ob)->{
            if(r-l>0 && b-t>0 && (availableWidth!=r-l || availableHeight!=b-t)){
                availableWidth=r-l; availableHeight=b-t; v.post(()->buildUI());
            }
        });
        setContentView(frame);
        refresh();
        mainUiContext=true; adaptViewBoxes(frame);
        mainUiContext=false;
    }
    ...
    void refresh(){
        ...
        if(presetRow!=null){
            ...
            if(!hasVisiblePreset){
                ...
                presetRow.addView(empty,new LinearLayout.LayoutParams(dp(200),dp(200)));
            }
        }
        ...
        if(appGrid!=null){
            ...
            if(apps.isEmpty()){
                ...
                appGrid.addView(empty,new LinearLayout.LayoutParams(dp(200),dp(200)));
            }else{
                PackageManager pm=getPackageManager();
                ...
                for(int index=0;index<apps.size();index++){
                    ...
                    if(inRow==0){
                        ...
                        row.setOrientation(LinearLayout.HORIZONTAL);
                        row.setGravity(Gravity.CENTER_VERTICAL);
                        appGrid.addView(row,new LinearLayout.LayoutParams(-1,dp(220)));
                    }
                    ...
                    inRow++;
                    int columns=Math.max(1,prefs.getInt("main_app_columns",8));
                    if(inRow>=columns) inRow=0;
                }
            }
        }
        if(mainFrame!=null)adaptViewBoxes(mainFrame);
        mainUiContext=false;
    }
    ...
    void placeDialogBelowTop(Dialog dialog){
        ...
        try{
            ...
            w.setBackgroundDrawable(new ColorDrawable(0xFF172332));
            w.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
            if(Build.VERSION.SDK_INT>=19) w.getDecorView().setSystemUiVisibility(0);
            WindowManager.LayoutParams lp=w.getAttributes();
            lp.gravity=Gravity.CENTER;
            lp.x=0; lp.y=0;
            Point area=availableUiSize();
            lp.width=Math.max(1,Math.min(dp(1000),area.x));
            lp.height=Math.max(1,Math.min(dp(700),area.y));
            lp.dimAmount=0.55f;
            w.setAttributes(lp);
            View content=dialog.findViewById(android.R.id.content);
            if(content!=null) content.setPadding(content.getPaddingLeft(),0,content.getPaddingRight(),content.getPaddingBottom());
    ...
    void showFixed900x960(AlertDialog dialog){
        ...
        styleDialogActionButtons(dialog);
        adaptDialogBoxes(dialog);
        // Legacy method name retained; shared 1000x700 host, no duplicate top inset.
        placeDialogBelowTop(dialog);
        Window w=dialog.getWindow();
        if(w!=null){
            WindowManager.LayoutParams lp=w.getAttributes();
    ...
    void showSettingsMenu(){
        ...
        // Keep the switch local until the settings Save action.
        ...
        Button settingsClose=button("取消"),settingsSave=button("保存");
        settingsActions.addView(settingsClose,new LinearLayout.LayoutParams(dp(100),dp(50)));
        settingsActions.addView(settingsSave,new LinearLayout.LayoutParams(dp(100),dp(50)));
        settingsSave.setOnClickListener(v->{prefs.edit().putBoolean("app_boot_enabled",appBoot.isChecked()).apply();settingsDialog[0].dismiss();});
    ...
    PreferenceDraft floatingDraft;
    Runnable refreshFloatingDraft;
    SharedPreferences floatingPreferences(){return floatingDraft==null?prefs:floatingDraft;}

    void showFloatingWindowSettingsDialog(){
        if(floatingDraft!=null)return;
        final PreferenceDraft draft=new PreferenceDraft(this.prefs);
        floatingDraft=draft;
        final SharedPreferences prefs=draft;
        // Dialog-local edits, including nested chooser changes, are committed only by Save.
        LinearLayout root=new LinearLayout(this);
        root.setOrientation(LinearLayout.VERTICAL);
        ...
        refreshCurrentButtons[0].run();
        refreshFloatingDraft=refreshCurrentButtons[0];
        ...
        Button shapeBtn=button("rounded".equals(prefs.getString("floating_single_icon_shape","rounded"))?"圆角正方形":"圆形");
        ...
        // Two readable columns on standard windows; one column when narrow.
        if(availableUiSize().x>=dp(900)){
            LinearLayout columns=new LinearLayout(this);
            LinearLayout left=new LinearLayout(this), right=new LinearLayout(this);
            left.setOrientation(LinearLayout.VERTICAL);right.setOrientation(LinearLayout.VERTICAL);
            int count=box.getChildCount();
            ArrayList<View> fields=new ArrayList<>();
            for(int i=0;i<count;i++)fields.add(box.getChildAt(i));
            box.removeAllViews();
            for(int i=0;i<count;i++){
                View field=fields.get(i);
                (i<9?left:right).addView(field,field.getLayoutParams());
            }
            LinearLayout.LayoutParams column=new LinearLayout.LayoutParams(0,-2,1);column.rightMargin=dp(20);
            columns.addView(left,column);columns.addView(right,new LinearLayout.LayoutParams(0,-2,1));
            box.addView(columns,new LinearLayout.LayoutParams(-1,-2));
        }
        ...
        dialog.setOnDismissListener(d->{floatingDraft=null;refreshFloatingDraft=null;});
        back.setText("取消");
        back.setOnClickListener(v->dialog.dismiss());
        save.setOnClickListener(v->{
            if(enable.isChecked() && !hasOverlayPermission()){
                Toast.makeText(this,"请先开启悬浮窗权限；草稿尚未保存",Toast.LENGTH_LONG).show();
            ...
            if(!draft.save()){
                Toast.makeText(this,"保存失败，请重试",Toast.LENGTH_LONG).show();return;
            }
            if(enable.isChecked()){
    ...
    void restartFloatingServiceSafe(){
        if(floatingDraft!=null){if(refreshFloatingDraft!=null)refreshFloatingDraft.run();return;}
        stopFloatingService();
        if(prefs.getBoolean("floating_enabled",false)) startFloatingService();
    }

    void showFloatingAppChooser(){
        final SharedPreferences prefs=floatingPreferences();
        ...
        presetScroll.addView(presetRow,new HorizontalScrollView.LayoutParams(-2,dp(48)));
        ...
        presetRow.addView(defaultPreset,new LinearLayout.LayoutParams(dp(86),dp(44)));
        ...
        addChooserSearch(root,allApps,appRows,category,selectedPkg);
        refreshFloatingChooserApps(allApps,appRows,category,selectedPkg);

        LinearLayout actions=new LinearLayout(this);
        actions.setGravity(Gravity.RIGHT|Gravity.CENTER_VERTICAL);
        actions.setPadding(dp(4),dp(8),dp(4),0);
        ...
        save.setOnClickListener(v->{
            ...
            try{
                ...
                for(int i=0;i<a.length();i++){
                    ...
                    if(o!=null&&pkg.equals(o.optString("pkg",""))){
    ...
    void addChooserSearch(LinearLayout root,ArrayList<ApplicationInfo> apps,LinearLayout rows,int[] category,String[] selected){
        EditText search=textField("搜索名称或包名","");
        root.addView(search,0,new LinearLayout.LayoutParams(-1,dp(50)));
        search.addTextChangedListener(new android.text.TextWatcher(){
            public void beforeTextChanged(CharSequence s,int a,int b,int c){}
            public void onTextChanged(CharSequence s,int a,int b,int c){
                rows.setTag(s.toString().trim().toLowerCase(Locale.ROOT));
                if(category[0]==3)refreshFloatingChooserButtons(rows,selected);
                else refreshFloatingChooserApps(apps,rows,category,selected);
            }
    ...
    void refreshFloatingChooserApps(ArrayList<ApplicationInfo> allApps, LinearLayout rows, int[] category, String[] selectedPkg){
        rows.removeAllViews();
        String query=rows.getTag() instanceof String?(String)rows.getTag():"";
        ...
        for(ApplicationInfo ai:allApps){
            ...
            if(!query.isEmpty() && !getAppLabelSafe(ai.packageName).toLowerCase(Locale.ROOT).contains(query)
                    && !ai.packageName.toLowerCase(Locale.ROOT).contains(query))continue;
            filtered.add(ai);
        }
        if(filtered.isEmpty()){
    ...
    void showInterfaceOptionsDialog(){
        ...
        EditText columnsInput=numberField("4",String.valueOf(prefs.getInt("main_app_columns",8)));
    ...
    void chooseApp(){
        ...
        Runnable refreshAppPicker=()->{
            ...
            int availableDp=Math.min(1000,Math.round(availableUiSize().x/uiScale()));
            int tileDp=Math.max(80,(availableDp-64)/columns-8);
            LinearLayout row=null;
            int inRow=0;
    ...
    void showPresetEditor(int index,Preset old){
        ...
        for(int m=1;m<=6;m++){
            ...
            mb.setOnClickListener(v->{
                modeHolder[0]=mm; // draft only: cancel must not mutate the stored preset
        ...
        save.setOnClickListener(v->{
            ...
            Preset p=new Preset(n,
                    ...
                    old.displayId,modeHolder[0],categoryHolder[0]);
