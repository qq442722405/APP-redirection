...
final class VoiceCommands {
    ...
    final int omitted;
    static List<Command> closeCommands(List<App> apps){
        ArrayList<Command> out=new ArrayList<>();for(App app:apps){if(app.pkg==null||app.pkg.isEmpty()||app.pkg.startsWith("action:"))continue;for(String raw:Arrays.asList(app.name,app.alias)){String name=normalize(raw);if(!name.isEmpty()&&name.length()<=198)out.add(new Command(app,"关闭"+name,"close"));}}return out;
    }
