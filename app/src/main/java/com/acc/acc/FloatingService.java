...
public class FloatingService extends Service {
    ...
    TextView baseButton(String label){
        ...
        g.setColor(DesignTheme.palette(this).button);
    ...
    void showPanel(){
        ...
        bg.setColor(DesignTheme.palette(this).panel);
        ...
        if(singleIconMode) {
            ...
            singleBg.setColor(DesignTheme.palette(this).button);
    ...
    void launchFloatingApp(String pkg){
        ...
        sp.edit().putString("selected_control_package",pkg).apply();
        Intent intent=getPackageManager().getLaunchIntentForPackage(pkg);if(intent==null)return;
        try{
            JSONObject bounds=null;boolean fullscreen=false;int displayId=wm.getDefaultDisplay().getDisplayId();
            int index=sp.getInt("floating_preset_"+pkg,-1);
            if(index>=0){
                JSONArray presets=new JSONArray(sp.getString(MainActivity.PRESETS,"[]"));
                bounds=presets.optJSONObject(index);
                if(bounds==null)throw new IllegalArgumentException("悬浮按钮的预设已失效，请重新选择");
                fullscreen=bounds.optInt("mode",1)==6;
                displayId=bounds.optInt("displayId",displayId);
            }else{
                bounds=new JSONObject(sp.getString("app_last_bounds","{}")).optJSONObject(pkg);
                if(bounds!=null){fullscreen=bounds.optBoolean("fullscreen",false);displayId=bounds.optInt("displayId",displayId);}
            }
            if(bounds!=null){
                if(displayId<0)displayId=wm.getDefaultDisplay().getDisplayId();
                android.graphics.Rect applied=WindowLaunch.start(this,intent,bounds.optInt("x"),bounds.optInt("y"),bounds.optInt("w"),bounds.optInt("h"),displayId,fullscreen,true);
                saveFloatingAppBounds(pkg,applied.left,applied.top,applied.width(),applied.height(),displayId,fullscreen);
            }else{WindowLaunch.prepare(intent,false);startActivity(intent);}
        }catch(Exception e){Toast.makeText(this,"启动失败："+e.getMessage(),Toast.LENGTH_LONG).show();}
    }
    ...
    void rebuildButtons(){
        ...
        for(int i=0;i<floatingPkgs.size();i++){
            ...
            installDraggableItem(b,()->launchFloatingApp(pkg),()->{
                try{startActivity(new Intent(this,MainActivity.class).putExtra("manage_floating",true).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP));}
                catch(RuntimeException e){Toast.makeText(this,"请打开启动器的悬浮窗口设置管理快捷键",Toast.LENGTH_SHORT).show();}
            });
    ...
    void performSelectedTargetAction(boolean close){
        ...
        try{
            WindowLaunch.activateForControl(this,pkg,launch);
    ...
    void showApps(){
        ...
        FrameLayout root=new FrameLayout(this);
        GradientDrawable bg=new GradientDrawable(); bg.setColor(DesignTheme.palette(this).panel); bg.setCornerRadius(dp(16));
        ...
        defaultPreset.setOnClickListener(v->{selectedPreset[0]=-1; defaultPreset.setBackground(DesignTheme.resource(this,R.drawable.card_selected)); for(int i=0;i<presetInner.getChildCount();i++)presetInner.getChildAt(i).setBackground(DesignTheme.resource(this,R.drawable.button));});
        for(int i=0;i<presetNames.size();i++){
            ...
            b.setOnClickListener(v->{selectedPreset[0]=pi; defaultPreset.setBackground(DesignTheme.resource(this,R.drawable.button)); for(int j=0;j<presetInner.getChildCount();j++)presetInner.getChildAt(j).setBackground(DesignTheme.resource(this,presetInner.getChildAt(j)==v?R.drawable.card_selected:R.drawable.button));});
        }
        ...
        for(int i=0;i<cats.length;i++){final int ci=i; Button b=button(cats[i]); DesignTypography.setPx(b,12*fontScale()); tabButtons[i]=b; tabs.addView(b,new LinearLayout.LayoutParams(0,dp(42),1)); b.setOnClickListener(v->{category[0]=ci; for(int j=0;j<tabButtons.length;j++)tabButtons[j].setBackground(DesignTheme.resource(this,j==ci?R.drawable.card_selected:R.drawable.button)); refreshApps[0].run();});}
        tabButtons[0].setBackground(DesignTheme.resource(this,R.drawable.card_selected)); box.addView(tabs,new LinearLayout.LayoutParams(-1,dp(44)));
        ...
        refreshApps[0]=()->{
            ...
            for(ApplicationInfo ai:list){
                ...
                LinearLayout tile=new LinearLayout(this);tile.setOrientation(LinearLayout.VERTICAL);tile.setGravity(Gravity.CENTER);tile.setPadding(dp(4),dp(4),dp(4),dp(4));tile.setBackground(DesignTheme.resource(this,R.drawable.floating_app_card));
