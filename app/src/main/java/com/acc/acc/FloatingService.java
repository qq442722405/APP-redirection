...
public class FloatingService extends Service {
        ...
        v.setOnTouchListener((view,event)->{
            switch(event.getActionMasked()){
                ...
                case MotionEvent.ACTION_CANCEL:
                    if(longPressRunnable!=null)gestureHandler.removeCallbacks(longPressRunnable);
                    if(singleTapRunnable!=null)gestureHandler.removeCallbacks(singleTapRunnable);
                    tapCount=0;longTriggered[0]=false;moved=false;lastDragDx=lastDragDy=0;
                    return true;
                case MotionEvent.ACTION_UP:
                    if(longPressRunnable!=null)gestureHandler.removeCallbacks(longPressRunnable);
                    if(longTriggered[0]){
                        if(singleTapRunnable!=null)gestureHandler.removeCallbacks(singleTapRunnable);
                        tapCount=0;longTriggered[0]=false;lastDragDx=lastDragDy=0;
                        return true;
                    }
    ...
    void rebuildButtons(){
        ...
        for(int i=0;i<floatingPkgs.size();i++){
            ...
            b.setGravity(Gravity.CENTER);
            installDraggableItem(b,()->launchFloatingApp(pkg),()->showRemoveConfirmation(pkg,displayName));
    ...
    void showRemoveConfirmation(String pkg,String displayName){
        if(Build.VERSION.SDK_INT>=23&&!Settings.canDrawOverlays(this)){
            Toast.makeText(this,"请先授权悬浮窗权限",Toast.LENGTH_SHORT).show();return;
        }
        removePanel();closeOverlay();
        LinearLayout root=overlayShell("删除悬浮快捷键");
        ScrollView scroll=new ScrollView(this);
        TextView message=overlayText("是否删除“"+displayName+"”这个悬浮窗口快捷键？\n仅删除快捷键，不会卸载应用。",18);
        message.setPadding(dp(12),dp(16),dp(12),dp(16));
        scroll.addView(message,new ScrollView.LayoutParams(-1,-2));
        root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));
        LinearLayout actions=new LinearLayout(this);
        Button cancel=overlayButton("取消"),remove=overlayButton("删除快捷键");
        cancel.setOnClickListener(v->{closeOverlay();showPanel();});
        remove.setOnClickListener(v->{removeFloatingApp(pkg);closeOverlay();showPanel();});
        LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(0,dp(48),1);cp.rightMargin=dp(8);
        actions.addView(cancel,cp);actions.addView(remove,new LinearLayout.LayoutParams(0,dp(48),1));
        root.addView(actions,new LinearLayout.LayoutParams(-1,-2));
        attachOverlay(root,"打开删除确认失败");
    }
     ...
     */
    // 1000 x 700 are reference pixels (DPI160), not launch-bound coordinates.
    int[] overlaySize(){
        android.util.DisplayMetrics dm=new android.util.DisplayMetrics();
        ...
        if(Build.VERSION.SDK_INT>=30){
            ...
            width=metrics.getBounds().width()-insets.left-insets.right;
            height=metrics.getBounds().height()-insets.top-insets.bottom;
        }
        int margin=Math.min(dp(16),Math.max(0,Math.min(width,height)/20));
        return new int[]{Math.max(1,Math.min(1000,width-2*margin)),Math.max(1,Math.min(700,height-2*margin))};
    }

    GradientDrawable overlayShape(int color){
        GradientDrawable shape=new GradientDrawable();
        shape.setColor(color);shape.setCornerRadius(dp(12));
        shape.setStroke(dp(1),0xFF40516A);
        return shape;
    }

    void styleOverlayControl(View view){
        android.graphics.drawable.StateListDrawable states=new android.graphics.drawable.StateListDrawable();
        states.addState(new int[]{android.R.attr.state_pressed},overlayShape(0xFF365780));
        states.addState(new int[]{android.R.attr.state_selected},overlayShape(0xFF285A94));
        states.addState(new int[]{android.R.attr.state_focused},overlayShape(0xFF365780));
        states.addState(new int[]{},overlayShape(0xFF263449));
        view.setBackground(states);view.setFocusable(true);
    }

    Button overlayButton(String text){
        Button b=button(text);styleOverlayControl(b);
    ...
    TextView overlayText(String text,int size){
        TextView label=new TextView(this);label.setText(text);
        label.setTextColor(0xFFE7EDF7);label.setTextSize(size*fontScale());
        return label;
    }

    LinearLayout overlayShell(String title){
        LinearLayout root=new LinearLayout(this){
            @Override public boolean dispatchKeyEvent(KeyEvent event){
                if(event.getKeyCode()==KeyEvent.KEYCODE_BACK && event.getAction()==KeyEvent.ACTION_UP){
                    closeOverlay();showPanel();return true;
                }
                return super.dispatchKeyEvent(event);
            }
        };
        root.setOrientation(LinearLayout.VERTICAL);root.setBackground(overlayShape(0xFF172231));
        root.setPadding(dp(12),dp(8),dp(12),dp(8));
        root.setFocusableInTouchMode(true);
        TextView heading=overlayText(title,22);heading.setSingleLine(true);
    ...
    void addOverlayFooter(LinearLayout root,String text,Runnable action){
        Button done=overlayButton(text);done.setOnClickListener(v->action.run());
        LinearLayout.LayoutParams params=new LinearLayout.LayoutParams(-1,dp(48));
        params.topMargin=dp(8);root.addView(done,params);
    }

    boolean attachOverlay(LinearLayout root,String failure){
        int[] size=overlaySize();overlayView=root;
        overlayLp=new WindowManager.LayoutParams(size[0],size[1],overlayType(),
                WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL|WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN,
                PixelFormat.OPAQUE);
        overlayLp.gravity=Gravity.CENTER;overlayLp.x=0;overlayLp.y=0;
        overlayLp.softInputMode=WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                |WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN;
        try{wm.addView(root,overlayLp);root.requestFocus();return true;}
        catch(Exception e){overlayView=null;Toast.makeText(this,failure,Toast.LENGTH_SHORT).show();showPanel();return false;}
    }
    ...
    void showAddMenu(){
        ...
        LinearLayout root=overlayShell("添加到悬浮窗口");
        ScrollView scroll=new ScrollView(this);
        ...
        root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));
        addOverlayFooter(root,"完成",()->{closeOverlay();showPanel();});
    ...
    void addMenuItem(LinearLayout parent,int icon,String title,String sub,View.OnClickListener listener){
        ...
        styleOverlayControl(row);
        ImageButton ib=iconButton(icon);
        ib.setBackgroundColor(Color.TRANSPARENT);ib.setClickable(false);ib.setFocusable(false);
        ib.setImportantForAccessibility(View.IMPORTANT_FOR_ACCESSIBILITY_NO);
        row.addView(ib,new LinearLayout.LayoutParams(dp(48),dp(48)));
        ...
        b.setTextColor(0xFFABB9CC);textBox.setPadding(dp(10),0,0,0);
        textBox.addView(a,new LinearLayout.LayoutParams(-1,-2));
        textBox.addView(b,new LinearLayout.LayoutParams(-1,-2));
        row.addView(textBox,new LinearLayout.LayoutParams(0,-2,1));
        row.setMinimumHeight(dp(76));row.setOnClickListener(listener);
        LinearLayout.LayoutParams item=new LinearLayout.LayoutParams(-1,-2);item.bottomMargin=dp(8);
        parent.addView(row,item);
    }
    ...
    void showApps(){
        ...
        LinearLayout root=overlayShell("添加 APP · 悬浮窗口");
        // All form controls scroll with the grid, so IME resize cannot hide the footer.
        ...
        addOverlayFooter(root,"取消",()->{closeOverlay();showPanel();});
        ...
        Button defaultPreset=overlayButton("默认");
        presetRow.addView(defaultPreset,new LinearLayout.LayoutParams(dp(92),dp(48)));
        HorizontalScrollView presetScroll=new HorizontalScrollView(this); presetScroll.setHorizontalScrollBarEnabled(false);
        LinearLayout presetInner=new LinearLayout(this); presetInner.setOrientation(LinearLayout.HORIZONTAL);
        presetScroll.addView(presetInner,new HorizontalScrollView.LayoutParams(-2,-1));
        presetRow.addView(presetScroll,new LinearLayout.LayoutParams(0,dp(48),1));
        box.addView(presetRow,new LinearLayout.LayoutParams(-1,dp(52)));
        ...
        defaultPreset.setSelected(true);
        defaultPreset.setOnClickListener(v->{selectedPreset[0]=-1;defaultPreset.setSelected(true);for(int i=0;i<presetInner.getChildCount();i++)presetInner.getChildAt(i).setSelected(false);});
        for(int i=0;i<presetNames.size();i++){
            final int pi=presetIndexes.get(i);Button b=overlayButton(presetNames.get(i));
            LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(dp(132),dp(48));pp.leftMargin=dp(6);
            presetInner.addView(b,pp);
            b.setOnClickListener(v->{selectedPreset[0]=pi;defaultPreset.setSelected(false);for(int j=0;j<presetInner.getChildCount();j++)presetInner.getChildAt(j).setSelected(presetInner.getChildAt(j)==v);});
        }
        ...
        for(int i=0;i<cats.length;i++){final int ci=i;Button b=overlayButton(cats[i]);tabButtons[i]=b;LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(0,dp(48),1);tp.setMargins(dp(2),0,dp(2),0);tabs.addView(b,tp);b.setOnClickListener(v->{category[0]=ci;for(int j=0;j<tabButtons.length;j++)tabButtons[j].setSelected(j==ci);refreshApps[0].run();});}
        tabButtons[0].setSelected(true);box.addView(tabs,new LinearLayout.LayoutParams(-1,dp(52)));
        ...
        search.setBackground(overlayShape(0xFF263449));search.setHintTextColor(0xFFABB9CC);
        search.setTextSize(16*fontScale());
        ...
        TextView resultLabel=overlayText("选择 APP 即添加；取消不更改配置",12);
        resultLabel.setPadding(dp(4),dp(8),dp(4),dp(8));box.addView(resultLabel);
        LinearLayout rows=new LinearLayout(this);rows.setOrientation(LinearLayout.VERTICAL);
        rows.setGravity(Gravity.CENTER_HORIZONTAL);box.addView(rows,new LinearLayout.LayoutParams(-1,-2));
        final int[] lastColumns={0};
        ...
        refreshApps[0]=()->{
            int available=rows.getWidth()>0?rows.getWidth():Math.max(1,overlaySize()[0]-root.getPaddingLeft()-root.getPaddingRight());
            int columns=Math.max(1,Math.min(5,available/dp(132)));
            lastColumns[0]=columns;
            ...
            for(ApplicationInfo ai:list){
                ...
                if(inRow==0){row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.TOP);row.setWeightSum(columns);rows.addView(row,new LinearLayout.LayoutParams(-1,-2));}
                LinearLayout tile=new LinearLayout(this);tile.setOrientation(LinearLayout.VERTICAL);tile.setGravity(Gravity.CENTER);tile.setPadding(dp(6),dp(8),dp(6),dp(8));styleOverlayControl(tile);tile.setMinimumHeight(dp(116));
                ...
                tile.addView(icon,new LinearLayout.LayoutParams(dp(48),dp(48)));TextView nv=overlayText(name,14);nv.setGravity(Gravity.CENTER);nv.setMinLines(2);nv.setMaxLines(2);nv.setEllipsize(android.text.TextUtils.TruncateAt.END);tile.addView(nv,new LinearLayout.LayoutParams(-1,-2));
                tile.setContentDescription(name+"，"+ai.packageName);
                ...
                if(row!=null)row.addView(tile,cell);inRow++;count++;if(inRow>=columns)inRow=0;
            }
        ...
        rows.addOnLayoutChangeListener((v,l,t,r,b,ol,ot,or,ob)->{
            int columns=Math.max(1,Math.min(5,(r-l)/dp(132)));
            if(r>l && columns!=lastColumns[0])rows.post(()->{if(overlayView==root)refreshApps[0].run();});
        });
        if(attachOverlay(root,"打开 APP 列表失败"))refreshApps[0].run();
    }
