...
public final class VoiceCommandsTest {
    ...
    public static void main(String[] args){
        ...
        List<VoiceCommands.App> music=Arrays.asList(app("a.music","音乐","听歌"));
        List<VoiceCommands.Command> close=VoiceCommands.closeCommands(music);
        VoiceCommands controls=new VoiceCommands(new VoiceCommands(music),close);
        check(controls.commands.size()==4,"open and close label/alias registered");
        check(controls.match("","关闭听歌").action.equals("close"),"close alias routes to close handler");
        VoiceCommands.Command closeId=controls.match("","关闭音乐");
        check(new VoiceCommands(music).match(closeId.id,closeId.phrase)==null,"disabled close command ID rejected");
        check(controls.match(new VoiceCommands(music).commands.get(0).id,"").action.equals("open"),"existing open IDs unchanged");
        VoiceCommands conflict=new VoiceCommands(empty,VoiceCommands.closeCommands(Arrays.asList(app("a.music","音乐",""),app("b.music","音乐",""))));
        check(conflict.commands.isEmpty(),"ambiguous close phrase rejected");
        check(new VoiceCommands(capped,close).commands.size()==500,"combined registration capped");
