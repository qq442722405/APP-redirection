...
public final class VoiceCommandsTest {
    ...
    public static void main(String[] args){
        ...
        VoiceCommands.App music=app("a.music","音乐","");
        VoiceCommands.Command close=new VoiceCommands.Command(music,"关闭音乐","lab.close");
        VoiceCommands experiments=new VoiceCommands(duplicates,Arrays.asList(close,close,new VoiceCommands.Command(music,"把音乐移到后排屏","lab.rear")));
        check(experiments.commands.size()==3,"experimental duplicates collapse alongside formal open command");
        check(experiments.match("","关闭音乐").action.equals("lab.close"),"close is an explicit experimental action");
        check(experiments.match("","把音乐移到后排屏").action.equals("lab.rear"),"rear screen distinct from right screen");
        check(experiments.match(duplicates.commands.get(0).id,"").action.equals("open"),"formal open IDs unchanged");
        check(new VoiceCommands(duplicates,Collections.emptyList()).match(close.id,close.phrase)==null,"withdrawn experiment IDs rejected");
        VoiceCommands conflictingActions=new VoiceCommands(empty,Arrays.asList(close,new VoiceCommands.Command(music,"关闭音乐","lab.kill")));
        check(conflictingActions.commands.isEmpty(),"same phrase mapped to different actions is rejected");
        VoiceCommands conflictingApps=new VoiceCommands(empty,Arrays.asList(close,new VoiceCommands.Command(app("b.music","音乐",""),"关闭音乐","lab.close")));
        check(conflictingApps.commands.isEmpty(),"same experimental phrase for different APPs rejected");
        check(new VoiceCommands(capped,Arrays.asList(close)).commands.size()==500,"combined registration respects limit");
